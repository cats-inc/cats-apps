...
# Usage: sync-agent-skills.sh [--project-root DIR] [--agent claude|codex|gemini] [--clean]
# Replaces matching canonical names only; preserves unrelated skills. Rejects symlinks.
set -e
usage() {
    echo "Usage: $0 [--project-root DIR] [--agent claude|codex|gemini] [--clean]"
...
case "$AGENT" in ""|claude|codex|gemini) ;; *) usage; exit 1 ;; esac
HOSTS=(.claude .agents .gemini)
case "$AGENT" in claude) HOSTS=(.claude) ;; codex) HOSTS=(.agents) ;; gemini) HOSTS=(.gemini) ;; esac
...
for host in "${HOSTS[@]}"; do
    ...
    for name in "${SKILLS[@]}"; do
        ...
        case "$target" in "$PROJECT_ROOT"/.claude/skills/*|"$PROJECT_ROOT"/.agents/skills/*|"$PROJECT_ROOT"/.gemini/skills/*) ;; *) exit 1 ;; esac
